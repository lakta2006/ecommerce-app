...
export const BottomNav: React.FC = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-light-bg dark:bg-dark-bg border-t border-light-border dark:border-dark-border z-50 safe-area-bottom shadow-lg dark:shadow-gray-900/50">
            ...
            <Link
              ...
              className={`flex flex-col items-center justify-center w-full h-full transition-all duration-200 relative active:scale-95 ${
                active
                  ...
                  : 'text-light-secondaryText dark:text-dark-text hover:text-light-text dark:hover:text-dark-text'
              }`}
            >
              {showBadge && (
              ...
              <span className={`text-xs mt-0.5 font-medium transition-all duration-200 ${
                active ? 'text-light-heading dark:text-dark-heading' : ''
              }`}>
                {item.label}
              </span>
            </Link>
