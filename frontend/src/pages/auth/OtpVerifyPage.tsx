    ...
    <AuthLayout title={title} subtitle={subtitle}>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex justify-center mb-6">
          <div className="w-16 h-16 rounded-full bg-light-border dark:bg-dark-border flex items-center justify-center">
            {type === 'email' ? (
              <Mail className="w-8 h-8 text-light-heading dark:text-dark-heading" />
            ) : (
              <Shield className="w-8 h-8 text-light-heading dark:text-dark-heading" />
            )}
          </div>
        </div>
        ...
        {email && (
          <div className="text-center mb-4">
            <p className="text-sm text-light-secondaryText dark:text-dark-text">جاري التحقق من:</p>
            <p className="font-medium text-light-text dark:text-dark-text">{email}</p>
          </div>
        )}
        ...
        <div className="flex justify-center gap-2">
          {otp.map((digit, index) => (
            <input
              ...
              onPaste={handlePaste}
              className="w-12 h-14 text-center text-2xl font-bold border-2 border-light-border dark:border-dark-border rounded-lg focus:border-light-heading focus:ring-2 focus:ring-light-heading outline-none transition-all bg-light-bg dark:bg-dark-bg text-light-text dark:text-dark-text"
              disabled={isLoading}
            />
          ))}
        </div>
        ...
        <div className="text-center">
          <p className="text-sm text-light-secondaryText dark:text-dark-text">
            ...
            <button
              ...
              className="text-light-link dark:text-dark-link hover:underline disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isResending
                ? 'جاري الإرسال...'
                : countdown > 0
